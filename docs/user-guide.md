...
Marketplace keeps public discovery in one compact panel near search. On first Marketplace entry, AgoraMesh may fetch AgoraMesh-native starter listings once from enabled relays when the synced cache is empty. After that, choose the AgoraMesh-only or all NIP-99 scope, then press Fetch to cache valid public records directly. Cards use a scan-friendly layout focused on image, title, price, location, seller, and source; open an item page for the image flipper, descriptions, contact, older payment instructions when present, receipts, trade actions, and owner publish options. Public relay publishing options are collapsed on local item pages. Public listing publishing uses NIP-99 classified listing events and makes the title, description, location, price, status, images, tags, and contact metadata readable by relays.
